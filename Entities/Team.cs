...
namespace Assets.FootballGameEngine_Indie.Scripts.Entities
{
    ...
    public class Team : MonoBehaviour
    {
        ...
        public Transform[] _midFieldPoints;
        public Transform[] _strikerFieldPoints;
        public Transform[] _defenderFieldPoints;
        public List<Player> kickOffPlayers = new List<Player>();
        public Transform goalKeeperPoint;
        public Vector3 goalPostPosition;
        public Vector3 oppPostPosition;
        public bool IsUserControlledTeam=false;
        public MatchManager matchManager;
        private Player currentUserControlledPlayer;
        ...
        public Player GoalKeeper { get => Players.Where(p => p.Player.PlayerType == PlayerTypes.Goalkeeper).Select(p => p.Player).FirstOrDefault(); }
        public List<InGamePlayerDto> Substitues = new List<InGamePlayerDto>();
        public List<TeamPlayer> Players = new List<TeamPlayer>();
        SortedDictionary<float, Player> playerDistancesToPost = new SortedDictionary<float, Player>();
        SortedDictionary<float, Player> playerDistancesToBall = new SortedDictionary<float, Player>();
        public Dictionary<Transform, Player> TeamPlayersDefaultPositionMap = new Dictionary<Transform, Player>();
        Dictionary<Player, float[]> playerIndexes = new Dictionary<Player, float[]>();
        ...
        private Player ballCurrentOwner;
        private bool hasSetUserPlayer;

        private void Awake()
        {
            hasSetUserPlayer = false;
        ...
        void Update()
        {
            if (!IsUserControlledTeam)
            {
                return;
            }
            ballCurrentOwner = Ball.Instance.CurrentOwner;
            if (ballCurrentOwner==null|| ballCurrentOwner.team != this)
            {
                Player nextUserControlledPlayer = getClosestPlayerToBall();
                if (nextUserControlledPlayer != null)
                {
                    if (currentUserControlledPlayer != null) currentUserControlledPlayer.IsUserControlled = false;
                    matchManager.UserControlledPlayer = nextUserControlledPlayer;
                    currentUserControlledPlayer = nextUserControlledPlayer;
                    nextUserControlledPlayer.IsUserControlled = true;
                    matchManager.SetDebugText(" closest player CHOSEN");
                }         
                else
                {
                    matchManager.SetDebugText("Null closest player");
                }
            }
            else
            {
                if (hasSetUserPlayer && ballCurrentOwner.team == this && ballCurrentOwner != currentUserControlledPlayer)
                {
                    Player nextUserControlledPlayerr = ballCurrentOwner;
                    if (nextUserControlledPlayerr != null)
                    {
                        if (currentUserControlledPlayer != null) currentUserControlledPlayer.IsUserControlled = false;
                        matchManager.UserControlledPlayer = nextUserControlledPlayerr;
                        currentUserControlledPlayer = nextUserControlledPlayerr;
                        nextUserControlledPlayerr.IsUserControlled = true;
                        matchManager.SetDebugText(" closest player CHOSEN");
                    }
                    return;
                }
                else if (hasSetUserPlayer) return;
                Player nextUserControlledPlayer = getClosestPlayerToBall();
                if (nextUserControlledPlayer != null)
                {
                    if (currentUserControlledPlayer != null) currentUserControlledPlayer.IsUserControlled = false;
        ...
        public void reportPlayerPosition(Player player)
        {
            ...
            float distanceToPost = (position - oppPostPosition).magnitude;
            float distanceToBall = (position - Ball.Instance.Position).magnitude;

            float[] formerValues;
            bool hasValue=playerIndexes.TryGetValue(player, out formerValues);
            if (hasValue)
            {
                playerDistancesToPost.Remove(formerValues[0]);
                playerDistancesToBall.Remove(formerValues[1]);
                playerIndexes.Remove(player);
            }
            if (!playerDistancesToPost.ContainsKey(distanceToPost))
            {
                playerDistancesToPost.Add(distanceToPost, player);
            }
            else
            {
                playerDistancesToPost[distanceToPost] = player;
            }

            if (!playerDistancesToBall.ContainsKey(distanceToBall))
                playerDistancesToBall.Add(distanceToBall, player);
            else
                playerDistancesToBall[distanceToBall] = player;

            float[] newVals = { distanceToPost, distanceToBall };
            playerIndexes.Add(player, newVals);
        }

        public Player getClosestPlayerToPost()
        {
            Player returnPlayer;
            bool hasMin = playerDistancesToPost.TryGetValue(playerDistancesToPost.Keys.First(), out returnPlayer);
            return returnPlayer;
        }

        public Player getClosestPlayerToPost(Player player)
        {
            Player returnPlayer=null;
            foreach(float key in playerDistancesToPost.Keys)
            {
                returnPlayer = playerDistancesToPost[key];
                if (playerDistancesToPost[key] != player) return returnPlayer;
            }
            return returnPlayer;
        }
        ...
        {
            Player returnPlayer;
            bool hasMin = playerDistancesToBall.TryGetValue(playerDistancesToBall.Keys.First(), out returnPlayer);
            return returnPlayer;
        }
        ...
        public void SetupNewPlayer(Player player)//Add Player To List, Assign Player To Point.
        {
            if (player.PlayerType == PlayerTypes.Striker)
            {
                foreach(Transform transform in _strikerFieldPoints)
                {
                    if (!TeamPlayersDefaultPositionMap.ContainsKey(transform))
                    {
                        kickOffPlayers.Add(player);
                        player.PutPlayerAtDefault(transform);
                        
                        TeamPlayersDefaultPositionMap[transform] = player;
                        break;
                    }
                }
            } else if (player.PlayerType == PlayerTypes.Defender)
            {
                foreach(Transform transform in _defenderFieldPoints)
                {
                    if (!TeamPlayersDefaultPositionMap.ContainsKey(transform))
                    {
                        player.PutPlayerAtDefault(transform);
            ...
            } else if (player.PlayerType == PlayerTypes.InFieldPlayer)
            {
                foreach(Transform transform in _midFieldPoints)
                {
                    if (!TeamPlayersDefaultPositionMap.ContainsKey(transform))
                    {
                        player.PutPlayerAtDefault(transform);
            ...
            } else if (player.PlayerType == PlayerTypes.Goalkeeper)
            {
                player.PutPlayerAtDefault(goalKeeperPoint);
                TeamPlayersDefaultPositionMap[goalKeeperPoint] = player;
            }
            reportPlayerPosition(player);
        }

        public void MoveAllPlayersToDefaultPoints()
        {
            foreach(Transform key in TeamPlayersDefaultPositionMap.Keys)
            {
                TeamPlayersDefaultPositionMap[key].MoveToDefaults();
            }
        }

        public void StopMovingToDefaultPoints()
        {
            foreach (Transform key in TeamPlayersDefaultPositionMap.Keys)
            {
                TeamPlayersDefaultPositionMap[key].StopMoveToDefaults();
            }
        }


        // Transform[] _midFieldPoints;
        //Transform[] _strikerFieldPoints;
        // Transform[] _defenderFieldPoints;
    ...
    public class TeamPlayer
    {
            ...
            InGameTeamDto teamDto)
        {
            ...
            Texture kit = playerIndex == 0 ? teamDto.Kit.GoalKeeperKit : teamDto.Kit.InFieldPlayerKit;
