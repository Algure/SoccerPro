...
namespace Assets.FootballGameEngine_Indie.Scripts.Entities
{
    public class Ball : Singleton<Ball>
    {
        ...
        /* if (_isGrounded)
         {
             _frictionVector *= _friction*-1;
             Rigidbody.AddRelativeForce(_frictionVector);
         }*/

         //apply friction if grounded
         //if (_isGrounded) //WAS IN ORIGINAL CODE
             Rigidbody.AddForce(_frictionVector*0.3f);
        ...
        private void DisplaceOwner()
        {
            OwnerWithLastTouch = CurrentOwner;
            CurrentOwner = null;
        }
        /// <summary>
        ...
        public void LaunchToPoint(Vector3 final, float power)
        {
           ...
           Vector3 initial = Position;
            ...
            float time = float.Parse((Math.Sqrt(toTargetXZ.magnitude / (20*power)).ToString()));
            updatePowerText(toTargetXZ.magnitude.ToString());
        ...
        void updatePowerText(String power)
        {
            if (_powerText != null)
                _powerText.text = "Distance:" + power;
        }
